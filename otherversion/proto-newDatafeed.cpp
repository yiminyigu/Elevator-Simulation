...
vector<int> ratioPeople{ -1,4,3,2,1 };//the ratio of idx number of people appears in this
//second, the ratio that 0 people appears should be calculated by TOTALTIME and FLOOR_PEOPLE_FEED
int ratioSum = 0;
...
void generateWaitGroupsForOneFloor(ofstream& ofs, int totalTime, int people, int currFloor, int totalFloor) {// 
	...
	for (int i = 0; i < totalTime; i++) {
		...
		for (int i = ratioPeople.size() - 1; i >= 0; i--) {
...
void feedData() {
	//calculate ratio of no people in vector ratioPeople
	int expectatationSum = 0;
	for (int i = 1; i < ratioPeople.size(); i++) {
		ratioSum += ratioPeople[i];
		expectatationSum += ratioPeople[i] * i;
	}
	ratioPeople[0] = (expectatationSum * TOTALTIME - ratioSum * FLOOR_PEOPLE_FEED) / FLOOR_PEOPLE_FEED;
	if (ratioPeople[0] < 0) {
		throw "FLOOR_PEOPLE_FEED range should be [0, 2*TOTALTIME]!";
	}
	//for this datafeed method, FLOOR_PEOPLE_FEED range should be [0, 2*TOTALTIME]
	ratioSum += ratioPeople[0];

	for (int i = 1; i < ratioPeople.size(); i++) {
		ratioPeople[i] += ratioPeople[i-1];
	}
