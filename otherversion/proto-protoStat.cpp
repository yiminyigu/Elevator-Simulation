...
public:
	...
	int upDateExternalReq(int floor, char direction, bool isStopping, int ExReq) {
		...
		
		{
			...
			if (numFloorMgrDone == TOTALFLOOR && requests_.empty()) {
				return CART_DONE;
			}
		}
...
public:
	...
	floorMgr() :time_(0), thisFloor_(count++), upRequest_(0), downRequest_(0), upWaitCount_(0), downWaitCount_(0)
	{
	...
	void operator()() {
		try {
			while (1) {
				...
				if (time_ >= TOTALTIME) {
					unique_lock<mutex> lock_floorData(mtxNumFloorMgrDone);
					numFloorMgrDone++;
					break;
				}
